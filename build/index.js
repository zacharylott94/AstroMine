...
const physicsLoop = () => {
    if (Controller.isButtonPushed("p"))
        humanInterface.pause();
    if (Controller.isButtonPushed("o"))
        humanInterface.reset();
    if (GameState.paused())
        return;
    if (Controller.isButtonHeld("w"))
        humanInterface.accelerate();
    if (Controller.isButtonHeld("a"))
        humanInterface.rotateCounterclockwise();
    if (Controller.isButtonHeld("d"))
        humanInterface.rotateClockwise();
    if (Controller.isButtonPushed("Enter"))
        humanInterface.fire();
