...
const generateParticleList = (generatorSettings) => {
    let particles = new Array(generatorSettings.number).fill(Particle(0));
...
const destroyParticleGenerator = timer => object => {
    ...
    const acceleration = offset => direction => {
        return time => Vector.fromDegreesAndMagnitude(mod((time * 42 * direction) + offset, 360), direction * 10 / Math.pow(time, 2.3));
    };
    return generateParticleList({
...
const DestroyParticles = particleMap(destroyParticleGenerator, obj => obj.type === 0 && obj.delete);
...
//# sourceMappingURL=ParticleEmitters.js.map